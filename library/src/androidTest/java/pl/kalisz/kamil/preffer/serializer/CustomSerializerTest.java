...
public class CustomSerializerTest extends ActivityInstrumentationTestCase2 {
    ...
    public void testCustomSerializeObjectWillProperSerializeAndDeserializeValue() {
    ...
    public void testWhenExceptionOccursWhenCreatingInstanceExceptionIsThrown() {
        CustomObjectPreferences customObjectPreferences = preffer.get(CustomObjectPreferences.class);

        try {
            customObjectPreferences.setlastViewedCategory("lastViewedCategory");
            Assert.fail();
        } catch (Exception e) {
            // do noting we expect fail
        }
    }
}
